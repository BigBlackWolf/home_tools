...
class Food(TestCase):
    ...
    def test_post_invalid_field(self):
        ...
        response = self.client.post("/food/", data=test_data, content_type="application/json")
    ...
    def test_post_without_measure(self):
        ...
        response = self.client.post("/food/", data=test_data, content_type="application/json")
        self.assertEqual(response.status_code, 422)
        self.assertEqual(response.json(), {'data': {'measure': ['Missing data for required field.']}})

    def test_post_successful(self):
        test_data = deepcopy(self.test_data)
        test_data["data"] = [test_data["data"]]

        response = self.client.post("/food/", data=self.test_data, content_type="application/json")
        self.assertEqual(response.status_code, 200)
...
class Dishes(TestCase):
    ...
    def test_post_invalid_field(self):
        ...
        response = self.client.post("/dishes/", data=test_data, content_type="application/json")
        self.assertEqual(response.status_code, 422)
        self.assertEqual(response.json(), {'data': {'abc': ['Unknown field.']}})
    ...
    def test_post_without_name(self):
        ...
        response = self.client.post("/dishes/", data=test_data, content_type="application/json")
        self.assertEqual(response.status_code, 422)
        self.assertEqual(response.json(), {'data': {'name': ['Missing data for required field.']}})

    def test_post_successful(self):
        test_data = deepcopy(self.test_data)
        test_data["data"] = [test_data["data"]]

        response = self.client.post("/dishes/", data=self.test_data, content_type="application/json")
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json(), test_data)
